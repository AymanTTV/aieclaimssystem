// src/pages/members/hooks/useMemberInvoices.ts
...
export function useMemberInvoices(customerId: string | null | undefined) {
  ...
  useEffect(() => {
    ...
    async function attach() {
      ...
      const unsubByCustomer = onSnapshot(
        ...
        (snap) => {
          if (cancelled) return;
          const list = new Map<string, MemberInvoice>();
    ...
    const mergeAndPush = (incoming: Map<string, MemberInvoice>) => {
      setInvoices((prev) => {
        const map = new Map<string, MemberInvoice>();
