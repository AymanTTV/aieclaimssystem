...
export default function IncomeExpense() {
    ...
    const processRecurring = async () => {
      ...
      for (const rec of dueRecords) {
        ...
        while (isBefore(currentDate, now)) {
            ...
            const newId = doc(collection(db, 'incomeExpenses')).id;
            const newRef = doc(db, 'incomeExpenses', newId);
        ...
        const oldRef = doc(db, 'incomeExpenses', rec.id);
  ...
  const historicalFilteredEntries = useMemo(() => {
    ...
    if (filter.filteredEntries) {
        // Since we are mixing custom logic (showShareHistory) with the hook logic,
        // we essentially re-apply what the hook does, OR just rely on the hook if we pass the right data set.
        // However, since the hook handles search, type, date range, AND RECURRING,
        // we can simplify this by passing the pre-filtered data to the hook? 
        // No, hooks order matters. 
        
        // Correct approach: The hook already filters 'records'. 
        // We just need to apply the 'isRecordSplitted' logic on top of the hook's result.
        
        data = filter.filteredEntries;
    }
    
    // Re-apply history filter because the hook doesn't know about it
    if (!showShareHistory) {
        data = data.filter(r => !isRecordSplitted(r, shares));
    }

    return data;
  }, [records, showShareHistory, shares, filter.filteredEntries]); // Depend on filter.filteredEntries
  ...
  const handleExport = () => { /* ... existing export ... */ };
  const handleGenerateDocument = async (entry: IncomeExpenseEntry) => { /* ... existing pdf ... */ };
  const handleExportBulkPDF = async () => { /* ... existing bulk ... */ };
  const handleDownloadProfitSharesPDF = async () => { /* ... existing shares ... */ };
  ...
  <div className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 className="text-[24px] font-bold text-[#0F172A] tracking-tight leading-tight">Income & Expenses</h1>
        <p className="text-sm text-[#64748B] mt-0.5 font-medium">Financial entries, operational expenses, recurring cash flow, and profit splits.</p>
      </div>

      <div className="flex flex-wrap items-center gap-2 justify-start sm:justify-end">
        {can('incomeExpense', 'categories') && (
    ...
    <Modal isOpen={showIncome} onClose={clearModals} title={recordBeingEdited ? 'Edit Income' : 'Add Income'} size="xl">
      <IncomeForm onClose={clearModals} record={recordBeingEdited?.type === 'income' ? recordBeingEdited : undefined} collectionName="incomeExpenses" categoriesCollection="incomeExpenseCategories" initialIsRecurring={isCreatingRecurring} />
    </Modal>

    <Modal isOpen={showExpense} onClose={clearModals} title={recordBeingEdited ? 'Edit Expense' : 'Add Expense'} size="xl">
      <ExpenseForm onClose={clearModals} record={recordBeingEdited?.type === 'expense' ? recordBeingEdited : undefined} collectionName="incomeExpenses" categoriesCollection="incomeExpenseCategories" initialIsRecurring={isCreatingRecurring} />
    </Modal>
