...
import ProductFormModal from '../products/ProductFormModal'; // Added Import
...
const VATRecordForm: React.FC<VATRecordFormProps> = ({
  ...
  const [pendingDescriptionIndex, setPendingDescriptionIndex] = useState<number | null>(null);
  ...
  const handleProductCreated = (product: any) => {
    ...
    if (pendingDescriptionIndex !== null && pendingDescriptionIndex < descriptions.length) {
       const descId = descriptions[pendingDescriptionIndex].id;
       updateDescription(descId, { 
         description: newSuggestion.name,
         net: newSuggestion.price
       });
       
       // Close dropdown for this row
       const newShow = [...showSuggestions]; 
       newShow[pendingDescriptionIndex] = false; 
       setShowSuggestions(newShow);
    }
    setPendingDescriptionIndex(null);
  };
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="relative col-span-1">
                  <FormField 
                    ...
                    onFocus={() => { const newShow = [...showSuggestions]; newShow[index] = true; setShowSuggestions(newShow); }} 
                    onBlur={() => { setTimeout(() => { const newShow = [...showSuggestions]; newShow[index] = false; setShowSuggestions(newShow); }, 200); }} 
                  ...
                  {showSuggestions[index] && desc.description && (
                    <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-lg mt-1 max-h-48 overflow-y-auto">
                      {/* Filter Matches */}
                      {productSuggestions
                        .filter(p => p.name.toLowerCase().includes(desc.description.toLowerCase()) || p.partNumber.toLowerCase().includes(desc.description.toLowerCase()))
                        .map((product) => ( 
                          <li key={product.id} className="px-4 py-2 cursor-pointer hover:bg-gray-100 flex items-center justify-between" onMouseDown={() => { updateDescription(desc.id, { description: product.name, net: product.price }); }}> 
                            <span className="truncate">{product.name}{product.partNumber && <span className="text-gray-500 text-sm"> — {product.partNumber}</span>}</span> 
                      ...
                      <li 
                        ...
                        onMouseDown={(e) => {
                          e.preventDefault(); 
                          setPendingDescriptionIndex(index);
