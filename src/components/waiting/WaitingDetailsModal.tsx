        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="rounded-2xl border border-slate-200 p-5 bg-white shadow-xs space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h3 className="text-xl font-black text-slate-900">{entry.fullName}</h3>
              <select
                id="modal-entry-status"
                className="px-3 py-1.5 bg-white text-[#0F172A] border-[1.5px] border-[#CBD5E1] rounded-xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer shadow-xs"
                value={entry.status}
                onChange={(e) => onStatusChange?.(e.target.value as WaitingStatus)}
              >
                {['new', 'contacted', 'waiting', 'offered', 'booked', 'not_proceeding'].map((s) => (
                  <option key={s} value={s} className="bg-white text-[#0F172A] py-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm">
              <div className="flex items-center gap-2 text-slate-500">
                ...
                <a className="text-slate-900 font-mono font-bold hover:underline" href={`tel:${entry.phone}`}>
                  {entry.phone}
                </a>
              </div>
              <div className="flex items-center gap-2 text-slate-500">
                <Mail className="w-4 h-4 text-indigo-400 shrink-0" />
                <span className="text-slate-900 font-medium truncate">{entry.email || '—'}</span>
              </div>
              <div className="flex items-center gap-2 text-slate-500">
              ...
              <div className="flex items-center gap-2 text-slate-500">
                ...
                <span className="text-slate-500">
                  Created: <strong className="text-slate-900 font-mono">{entry.createdAt ? format(entry.createdAt, 'dd/MM/yyyy HH:mm') : '—'}</strong>
                </span>
              </div>
              <div className="flex items-center gap-2 text-slate-500">
                <Clock className="w-4 h-4 text-rose-400 shrink-0" />
                <span className="text-slate-500">
                  Offer expiry: <strong className="text-amber-700 font-mono font-semibold">{entry.offerExpiryAt ? format(entry.offerExpiryAt, 'dd/MM/yyyy HH:mm') : 'None'}</strong>
                </span>
              </div>
            </div>

            <div className="pt-2">
              <div className="text-xs uppercase font-bold text-slate-700 tracking-wider mb-2">Reason / Notes</div>
              <div className="rounded-xl border border-slate-200 p-3.5 bg-slate-50 text-slate-800 text-xs sm:text-sm leading-relaxed whitespace-pre-wrap font-medium shadow-xs">
                {(entry.reason || 'No initial reason provided')}{entry.preferredNotes ? `\n\n${entry.preferredNotes}` : ''}
              </div>
            </div>

            <div className="pt-2">
              <div className="text-xs uppercase font-bold text-slate-700 tracking-wider mb-2">Quick Dispatch Actions</div>
              {contactBlock}
            </div>
          </div>

          {/* Notes Card */}
          <div className="rounded-2xl border border-slate-200 p-5 bg-white shadow-xs space-y-3.5">
            <div className="flex items-center justify-between">
              <h4 className="font-bold text-slate-900 text-sm">Driver Notes ({notes.length})</h4>
            </div>

            <div className="space-y-2.5 max-h-56 overflow-y-auto pr-1">
              {notes.map((n) => (
                <div key={n.id} className="border border-slate-200 rounded-xl p-3 bg-slate-50 shadow-xs space-y-1">
                  <div className="text-xs sm:text-sm text-slate-800 font-medium whitespace-pre-wrap leading-relaxed">{n.text}</div>
            ...
            <div className="pt-2 flex flex-col sm:flex-row gap-2">
              <textarea
                ...
                className="flex-1 bg-white border-[1.5px] border-[#CBD5E1] text-[#0F172A] placeholder-slate-400 rounded-xl p-3 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none min-h-[64px] shadow-xs"
          ...
          <div className="rounded-2xl border border-slate-200 p-5 bg-white shadow-xs space-y-3.5">
            <div className="flex items-center justify-between">
              <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                <Bell className="w-4 h-4 text-amber-500" /> Reminders ({reminders.length})
              </h4>
            </div>
            ...
            <div className="space-y-2.5 max-h-64 overflow-y-auto pr-1">
              {reminders.map((r) => (
                <div
                  key={r.id}
                  className={`border rounded-xl p-3 bg-slate-50 shadow-xs transition border-slate-200 ${
                    r.isDone ? 'border-slate-100 opacity-70' : 'border-slate-200'
                  }`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className={`text-xs sm:text-sm font-semibold text-slate-900 leading-snug ${r.isDone ? 'line-through text-slate-400' : ''}`}>
                      {r.message}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <label className="text-xs text-slate-500 flex items-center gap-1.5 cursor-pointer select-none">
                        ...
                        />
                        <span className="text-slate-700 text-xs font-bold">Done</span>
                      </label>
                      <button
                  ...
                  <div className="mt-1.5 flex items-center justify-between text-[11px] text-slate-400">
                    <span className="text-amber-700 font-mono font-semibold font-medium">
            ...
            <div className="pt-2 space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white border-[1.5px] border-[#CBD5E1] text-[#0F172A] placeholder-slate-400 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-xs"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white border-[1.5px] border-[#CBD5E1] text-[#0F172A] rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-xs"
                />
              </div>
              <button
