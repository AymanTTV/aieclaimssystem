...
const FinanceFilters: React.FC<FinanceFiltersProps> = ({
  ...
  const ownerOptions = useMemo(
    () => [{ id: 'all', label: 'All Owners' }, { id: 'no_owner_assigned', label: 'No Vehicle Assigned' }, ...owners.map((o) => ({ id: o, label: o }))],
    [owners]
  );

  const customerOptions = useMemo(
    () => [{ id: 'all', label: 'All Customers' }, ...customers.map((c) => ({ id: c.id, label: c.name }))],
    [customers]
  );
  ...
  // Account filter rules: [] means "DEFAULT (unassigned-only)" NOT "all"
  ...
  const handleAccountChange = (val: any) => {
    // When cleared via X, force back to [] (unassigned-only mode)
    if (val == null) return onAccountFilterChange([]);

    // Multi select normally returns array
    if (Array.isArray(val)) {
      const cleaned = val.filter(Boolean);

      // ✅ If user selects "All Accounts", treat it as an explicit value.
      // If "all" is present, keep only "all" (and optionally "no_account_assigned")
      // so we don't end up with "all + specific ids" which is ambiguous.
      ...
      // Normal multi-select: keep chosen ids (and optionally no_account_assigned)
  ...
  isMulti={true}
  multiEmptyMode="empty"          // ✅ IMPORTANT: keep [] as []
  showAllChipInMulti={true}       // ✅ show "All Accounts" visibly
  allId="all"
/>
