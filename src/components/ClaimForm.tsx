...
interface ClaimFormProps {
  onClose: () => void;
  claim?: any; // Add proper type
}

const ClaimForm: React.FC<ClaimFormProps> = ({ onClose, claim }) => {
  const { user } = useAuth();
  ...
  const [formData, setFormData] = useState({
    ...
    insuranceRef: claim?.insuranceRef || '',
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end space-x-3">
        ...
        >
          {loading ? 'Submitting...' : claim ? 'Update Claim' : 'Submit Claim'}
        </button>
      </div>
    </form>
  );
