...
interface ManageClaimDepartmentsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const ManageClaimDepartmentsModal: React.FC<ManageClaimDepartmentsModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchDepartments = async () => {
      const snap = await getDocs(collection(db, 'claimDepartments'));
  ...
  const handleAddDepartment = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const docRef = await addDoc(collection(db, 'claimDepartments'), { name: trimmed, createdAt: serverTimestamp() });
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    setLoading(true);
    try {
      await deleteDoc(doc(db, 'claimDepartments', id));
  ...
  const handleSaveEdit = async () => {
    ...
    setLoading(true);
    try {
      await updateDoc(doc(db, 'claimDepartments', editingId), { name: editingName.trim() });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Manage Claim Departments" size="md">
                ...
                {editingId === dept.id ? (
                  ...
                  <>
                    ...
                    <div className="flex space-x-2">
                      <button onClick={() => { setEditingId(dept.id); setEditingName(dept.name); }} className="text-blue-600 hover:text-blue-800"><Edit2 className="h-4 w-4" /></button>
                      <button onClick={() => handleDelete(dept.id, dept.name)} className="text-red-600 hover:text-red-800"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </>
                )}
              </li>
...
export default ManageClaimDepartmentsModal;