...
export const claimFormSchema = z
  ...
  .superRefine((data, ctx) => {
    //
    // ★ COMMENTED OUT OLD ▶︎ if (data.claimReason.includes('H')) { … } / 'S' blocks
    //
    ...
    // ▶︎ NEW: only enforce fields when the user has toggled each section’s `enabled` flag
    ...
    if (data.hireDetails?.enabled) {
      ...
      // …you can add more field checks here as needed…
    }
    ...
    if (data.storage?.enabled) {
      ...
      // …add any other storage-field validations here…
    }
  ...
  // Only validate GP information if PI is selected
    ...
    if (data.claimReason.includes('PI')) {
      if (data.gpInformation.visited === true) { // Check if visited is true (Yes)
        // If GP was visited, validate the other fields
        ...
        if (!data.gpInformation.gpAddress) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "GP Address is required",
        ...
        if (!data.gpInformation.gpDoctorName) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "GP Doctor Name is required",
      ...
      } else if (data.gpInformation.visited === false) { // Check if visited is false (No)
        // If GP was not visited, do nothing (no validation needed)
      } else {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
      ...
      if (data.hospitalInformation.visited === true) {
        ...
        if (!data.hospitalInformation.hospitalDoctorName) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Hospital Doctor Name is required",
        ...
        if (!data.hospitalInformation.hospitalDate) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Hospital Date is required",
      ...
      } else if (data.hospitalInformation.visited === false) {
        // do nothing
      } else {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
