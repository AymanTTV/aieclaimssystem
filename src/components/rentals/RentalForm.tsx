...
const RentalForm: React.FC<RentalFormProps> = ({ vehicles, customers, onClose }) => {
  ...
  const remainingAmount = finalCostAfterDiscount - (formData.paidAmount || 0);
...
await updateDoc(doc(db, 'rentals', docRef.id), { checkOutCondition });
      ...
      // Generate & upload docs (background)
      setTimeout(async () => {
        try {
      ...
      // Finance transaction (background)
      if (formData.paidAmount > 0) {
        setTimeout(async () => {
          try {
              ...
              <FormField
                ...
                onChange={(e) => setFormData(prev => {
                  ...
                  if (startDateObj && endDateObj && isValid(startDateObj) && isValid(endDateObj) && !isAfter(startDateObj, endDateObj)) {
                    ...
                    if (newNumberOfWeeks < 1) newNumberOfWeeks = 1;
                  }
        ...
        <div className="bg-gray-50 p-4 rounded-lg space-y-2">
          ...
          {formData.type === 'claim' && formData.recoveryCost > 0 && (
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium">{formatCurrency(formData.recoveryCost * (formData.includeRecoveryCostVAT ? 1.2 : 1))}</span>
            </div>
          )}
