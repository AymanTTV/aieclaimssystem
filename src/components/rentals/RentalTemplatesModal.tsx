      ...
      >
        <div className="flex flex-col h-full space-y-3 text-slate-800 min-h-0">
          ...
          <div className="shrink-0 space-y-3">
              ...
              <div className="flex space-x-2">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 text-sm font-bold transition-all border-b-2 cursor-pointer ${
                    activeTab === 'whatsapp'
                      ? 'border-emerald-600 text-emerald-700 bg-emerald-50 rounded-t-lg shadow-xs'
                      : 'border-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-t-lg'
                  }`}
                >
                  <div className={`p-1 rounded-md ${activeTab === 'whatsapp' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 text-emerald-600'}`}>
                    <MessageCircle className="w-4 h-4" />
                  </div>
                  <span>WhatsApp Templates</span>
                  <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                    activeTab === 'whatsapp' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-600'
                  }`}>
                    {whatsappCount}
                  </span>
                </button>
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 text-sm font-bold transition-all border-b-2 cursor-pointer ${
                    activeTab === 'email'
                      ? 'border-sky-600 text-sky-700 bg-sky-50 rounded-t-lg shadow-xs'
                      : 'border-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-t-lg'
                  }`}
                >
                  <div className={`p-1 rounded-md ${activeTab === 'email' ? 'bg-sky-600 text-white' : 'bg-sky-50 text-sky-600'}`}>
                    <Mail className="w-4 h-4" />
                  </div>
                  <span>Email Templates</span>
                  <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                    activeTab === 'email' ? 'bg-sky-100 text-sky-800' : 'bg-slate-200 text-slate-600'
                  }`}>
                    {emailCount}
                  </span>
                </button>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2.5 bg-slate-50 border border-slate-200 p-2.5 rounded-xl">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-1.5 text-xs bg-white text-[#0F172A] border-[1.5px] border-[#CBD5E1] rounded-lg focus:outline-none focus:border-indigo-500 placeholder-slate-400 shadow-xs"
                />
              </div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <select
                  ...
                  className="px-2.5 py-1 text-xs bg-white text-[#0F172A] border-[1.5px] border-[#CBD5E1] rounded-lg focus:outline-none focus:border-indigo-500 shadow-xs"
                >
                ...
                <button
                  ...
                  className="p-1.5 text-slate-600 bg-white hover:bg-slate-100 border border-slate-200 rounded-lg cursor-pointer disabled:opacity-50 transition-colors shadow-xs"
                >
          ...
          <div className="flex-1 min-h-0 overflow-y-auto pr-1 space-y-3 custom-scrollbar">
            {loading ? (
              ...
              <div className="text-center py-10 bg-slate-50 border border-slate-200 rounded-xl p-6">
                ...
                <h5 className="text-sm font-bold text-slate-900 mb-1">No templates found</h5>
              ...
              filteredTemplates.map((t) => {
                ...
                return (
                  <div
                    key={t.id || t.name}
                    className="bg-white border border-slate-200 hover:border-slate-300 rounded-xl p-3.5 transition-all space-y-2.5 shadow-xs"
                  >
                    {/* Header */}
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {/* Category Badge */}
                          <span className="px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider bg-indigo-50 text-indigo-700 border border-indigo-200">
                            {t.category || 'Rental'}
                          </span>
          ...
          <div className="shrink-0 flex items-center justify-between pt-2.5 border-t border-slate-200">
            <div className="text-xs text-slate-400">
              Showing <strong className="text-slate-900">{filteredTemplates.length}</strong> {activeTab} templates
            </div>
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-1.5 text-sm font-semibold text-slate-700 bg-white hover:bg-slate-100 border border-slate-200 rounded-lg cursor-pointer transition-colors shadow-xs"
            >
              Close
            </button>
          </div>
