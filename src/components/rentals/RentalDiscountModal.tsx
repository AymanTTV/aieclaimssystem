...
const RentalDiscountModal: React.FC<RentalDiscountModalProps> = ({
  ...
  try {
    ...
    const discountAmount = (rental.cost * formData.discountPercentage) / 100;
    
    // Calculate free period discount using vehicle's actual rate
    const freePeriodDiscount = formData.freePeriod * vehicleRate;

    // Calculate total discount and remaining amount
    ...
    await updateDoc(doc(db, 'rentals', rental.id), {
      ...
      updatedAt: new Date(),
      updatedBy: user.id,
      // Append new discount history entry to existing array
  ...
  const standardDiscountAmount = (rental.cost * formData.discountPercentage) / 100;
  const freePeriodDiscount = formData.freePeriod * vehicleRate;
  const totalDiscountAmount = standardDiscountAmount + freePeriodDiscount;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="bg-gray-50 p-4 rounded-lg space-y-2">
        ...
        <div className="text-sm text-gray-500 pt-2 border-t">
          <div>Vehicle {rental.type === 'weekly' ? 'Weekly' : 'Daily'} Rate: 
            £{vehicleRate}
          </div>
        </div>
      </div>
      ...
      <FormField
        ...
        onChange={(e) => setFormData({ ...formData, freePeriod: parseInt(e.target.value) })}
        min="0"
        max={rental.type === 'weekly' ? 52 : 365}
        step="1"
      />

      {(formData.discountPercentage > 0 || formData.freePeriod > 0) && (
        <div className="bg-blue-50 p-4 rounded-lg space-y-2">
          {formData.discountPercentage > 0 && (
            <div className="flex justify-between text-sm">
              <span>Percentage Discount:</span>
              <span className="text-blue-600">£{standardDiscountAmount.toFixed(2)}</span>
            </div>
          )}
          {formData.freePeriod > 0 && (
            <div className="flex justify-between text-sm">
              <span>Free Period Discount ({formData.freePeriod} {rental.type === 'weekly' ? 'weeks' : 'days'} @ £{vehicleRate}):</span>
