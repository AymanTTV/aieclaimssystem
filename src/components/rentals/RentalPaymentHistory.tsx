...
const RentalPaymentHistory: React.FC<RentalPaymentHistoryProps> = ({
  ...
  const getMethodIcon = (method: string) => {
    ...
    if (m.includes('card')) return <CreditCard className="w-4 h-4 text-blue-600" />;
    if (m.includes('cash')) return <Banknote className="w-4 h-4 text-emerald-600" />;
    if (m.includes('transfer') || m.includes('bank')) return <Building className="w-4 h-4 text-purple-600" />;
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2.5">
        {payments.map((payment) => (
          <div key={payment.id} className="bg-white border border-slate-200 hover:border-slate-300 transition-colors p-4 rounded-xl shadow-xs">
              ...
              <div className="space-y-1">
                ...
                {payment.allocatedVehicleName && (
                  <div className="text-xs text-blue-600 font-medium">
                    Allocated Vehicle: <span className="text-slate-900 font-bold">{payment.allocatedVehicleName}</span>
                  </div>
                )}

                {payment.reference && (
                  <div className="text-xs text-slate-600">
                    Ref: <span className="font-mono text-slate-800 font-semibold">{payment.reference}</span>
                  </div>
                )}

                {payment.notes && (
                  <div className="text-xs text-slate-600 italic bg-slate-50 p-2 rounded-lg border border-slate-200 mt-1">
                    "{payment.notes}"
                  </div>
                )}
              </div>
              ...
              <div className="flex sm:flex-col items-center sm:items-end justify-between gap-2 shrink-0">
                ...
                {payment.document && onDownloadDocument && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold rounded-lg border border-blue-200 transition-colors cursor-pointer shadow-xs"
