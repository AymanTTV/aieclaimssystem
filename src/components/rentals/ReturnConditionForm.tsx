...
const ReturnConditionForm: React.FC<ReturnConditionFormProps> = ({
  ...
  const calculateCharges = (): number => {
    ...
    if (formData.hasDamage) {
      total += formData.damageCost;
    }
    if (parseInt(checkOutCondition.fuelLevel) - parseInt(formData.fuelLevel) > 0) {
      total += formData.fuelCharge;
    }
    if (!formData.isClean) {
      total += formData.cleaningCharge;
    }
    return total;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const conditionData: Omit<ReturnCondition, 'id' | 'createdAt' | 'createdBy'> = {
        ...
        images: imageUrls,
        type: 'check-in',
        damageCost: formData.hasDamage ? formData.damageCost : 0,
        fuelCharge: formData.fuelCharge > 0 ? formData.fuelCharge : 0,
        cleaningCharge: !formData.isClean ? formData.cleaningCharge : 0,
