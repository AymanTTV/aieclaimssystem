...
const VDFinanceFilters: React.FC<VDFinanceFiltersProps> = ({
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] p-4 sm:p-5 rounded-2xl shadow-xs text-slate-900 space-y-4">
        ...
        <div className="relative flex-1">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3.5 py-2.5 border-[1.5px] border-[#CBD5E1] bg-white text-[#0F172A] rounded-xl text-sm placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all"
            />
          </div>
        </div>
        <div className="w-full sm:w-56">
          <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Profit Status</label>
          <select
            value={statusFilter}
            onChange={e => onStatusChange(e.target.value as ProfitStatusFilter)}
            className="block w-full px-3 py-2.5 text-sm font-medium text-[#0F172A] bg-white border-[1.5px] border-[#CBD5E1] rounded-xl focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-4">
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Record From</label>
            <input
              type="date"
              value={startStr}
              max={endStr || undefined}
              onChange={e => onDateRangeChange({ ...dateRange, start: e.target.value ? new Date(e.target.value) : null })}
              className="w-full bg-white border-[1.5px] border-[#CBD5E1] text-[#0F172A] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Record To</label>
            <input
              type="date"
              value={endStr}
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Incident From</label>
            <input
              type="date"
              value={incStartStr}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Incident To</label>
            <input
              type="date"
              value={incEndStr}
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Min Total (£)</label>
            <input
              type="number"
              step="0.01"
              min="0"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Max Total (£)</label>
            <input
              type="number"
              step="0.01"
