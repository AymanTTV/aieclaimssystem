// src/components/vdFinance/ManageVDFinanceGroupsModal.tsx
...
const ManageVDFinanceGroupsModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const addItem = async () => {
    ...
    setLoading(true);
    try {
      await addDoc(collection(db, 'vdFinanceGroups'), { name, createdAt: serverTimestamp() });
  ...
  const saveEdit = async () => {
    ...
    setLoading(true);
    try {
      await updateDoc(doc(db, 'vdFinanceGroups', editingId), { name });
  ...
  const remove = async (id: string, name: string) => {
    ...
    setLoading(true);
    try {
      await deleteDoc(doc(db, 'vdFinanceGroups', id));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">Manage VD Finance Groups</h2>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-md"><X className="h-5 w-5" /></button>
        </div>

        <div className="flex gap-2 mb-4">
          <input className="flex-1 form-input" placeholder="New group" value={newName} onChange={e => setNewName(e.target.value)} />
        ...
        <div className="divide-y border rounded-md">
          ...
          {items.length === 0 && <div className="p-3 text-sm text-gray-500">No groups yet.</div>}
        </div>
      </div>
    </div>
  );
...
export default ManageVDFinanceGroupsModal;
