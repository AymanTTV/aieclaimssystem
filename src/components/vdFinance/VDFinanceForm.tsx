        ...
        <div className="flex items-center justify-between">
          ...
          <label className="flex items-center space-x-2">
            <input
              type="checkbox"
              checked={manualEntry}
              onChange={(e) => { setManualEntry(e.target.checked); if (e.target.checked) setSelectedClaim(null) }}
            ...
            <div key={index} className="grid grid-cols-1 sm:grid-cols-5 gap-4 items-end p-3 border border-gray-200 rounded-md bg-gray-50">
              <div className="relative col-span-1 sm:col-span-2">
                ...
                {showPartSuggestions[index] &&
                  part.name &&
                  partSuggestions.filter((s) => s.name.toLowerCase().includes(part.name.toLowerCase())).length > 0 && (
                    <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-lg mt-1 max-h-48 overflow-y-auto">
                      {partSuggestions
                        ...
                        .map((sugg, i) => (
                          ...
                          >
                            {sugg.name} <span className="text-gray-500 text-sm">(Last price: {formatCurrency(sugg.lastPrice)})</span>
                          </li>
                        ))}
                    </ul>
                  )}
      ...
      <div>
        ...
        <div className="flex items-center space-x-2 mt-1">
          ...
          <span className="py-2 font-medium text-gray-800">
            = {formatCurrency(includeVATOnLabor ? formData.laborHours * formData.laborRate * 1.2 : formData.laborHours * formData.laborRate)}
          </span>
        </div>
      </div>
      <div>
        ...
        <ServiceCenterDropdown
          ...
          onChange={(center) => setFormData((prev) => ({ ...prev, serviceCenter: center.name, laborRate: center.hourlyRate }))}
