// src/components/vdFinance/AssignVDFinanceGroupModal.tsx
...
interface AssignVDFinanceGroupModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const AssignVDFinanceGroupModal: React.FC<AssignVDFinanceGroupModalProps> = ({ isOpen, onClose, selectedIds, groups, onSuccess }) => {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      selectedIds.forEach(id => {
        batch.update(doc(db, 'vdFinance', id), { groupId: selectedGroup, groupName });
      });

      await batch.commit();
      toast.success(`Assigned ${selectedIds.size} records to group`);
      onSuccess();
    } catch (error) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Assign ${selectedIds.size} Records to Group`} size="sm">
...
export default AssignVDFinanceGroupModal;