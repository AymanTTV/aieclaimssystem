...
const VDInvoiceForm: React.FC<VDInvoiceFormProps> = ({
  ...
  const handleProductCreated = (product: any) => {
    ...
    setPartSuggestions(prev => [...prev, newSuggestion]);

    // If we have a pending index, fill that line item
    if (pendingPartIndex !== null && pendingPartIndex >= 0 && pendingPartIndex < formData.parts.length) {
      const newParts = [...formData.parts];
      newParts[pendingPartIndex] = {
        ...newParts[pendingPartIndex],
        name: newSuggestion.name,
        price: newSuggestion.lastPrice
      };
  ...
  return (
    <>
      {/* New Product Modal (Outside form) */}
      <ProductFormModal 
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Parts Section (with Discount + VAT + Create Product) */}
        <div>
                ...
                <div className="relative col-span-1 sm:col-span-2">
                  ...
                  {showPartSuggestions[index] && part.name && (
                    <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-lg mt-1 max-h-48 overflow-y-auto">
                      {/* Existing Products Matches */}
                      {partSuggestions
                        .filter(s =>
                          s.name.toLowerCase().includes(part.name.toLowerCase()) || 
                          s.partNumber.toLowerCase().includes(part.name.toLowerCase())
                        )
                        .map(s => (
                          <li
                            key={s.id}
                            ...
                            onMouseDown={() => {
                              ...
                              newParts[index] = {
                                ...part,
                                name: s.name,
                                price: s.lastPrice, // autofill unit price
                              };
                              setFormData({ ...formData, parts: newParts });
