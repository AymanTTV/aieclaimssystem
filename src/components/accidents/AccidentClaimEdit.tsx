...
const AccidentClaimEdit: React.FC<AccidentClaimEditProps> = ({ accident, onClose }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    reportedDate: accident.reportedDate || (accident.submittedAt ? new Date(accident.submittedAt).toISOString().split('T')[0] : ''),
    reportedTime: accident.reportedTime || (accident.submittedAt ? new Date(accident.submittedAt).toTimeString().substring(0, 5) : ''),
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      {/* Top Action Bar for Immediate Save / Update */}
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-3.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-semibold text-slate-800">
              Editing Claim: {formData.referenceName || formData.referenceNo || 'Accident Claim'}
            </span>
          </div>
          <p className="text-xs text-slate-500 mt-0.5">
            Progressively update insurance response, outside settlements, and financial tracking at any time.
          </p>
        </div>
        <button
          type="submit"
        ...
        >
          ...
          <span>{loading ? 'Saving...' : 'Update Claim'}</span>
        </button>
      </div>
      ...
      <div className="space-y-4 border-t pt-6">
        ...
        <p className="text-sm text-gray-500">
          Track insurer communications, claims reference numbers, and received response dates.
        </p>
