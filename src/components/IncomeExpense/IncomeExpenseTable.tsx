...
interface Props {
  ...
  shares?: ProfitShare[]; // Added shares prop
...
const IncomeExpenseTable: React.FC<Props> = ({
  ...
  // Helper to check if record is splitted
  ...
  const columns = [
        ...
        return (
          <div className="relative">
            <div className="font-medium text-gray-900 flex items-center gap-2">
              ...
              {covered && (
                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-purple-100 text-purple-800 border border-purple-200" title="This record is included in a profit share">
                  Splitted
                </span>
              )}
            </div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
            <span className={`font-semibold ${isIncome ? 'text-green-700' : 'text-red-700'}`}>
                {typeof total === 'number' ? formatCurrency(total) : '—'}
            </span>
        );
      }
    },
    {
      ...
      cell: ({ row }) => (
         <span className={`text-xs px-2 py-1 rounded-full ${
             row.original.status === 'Paid' ? 'bg-green-50 text-green-700' : 
             row.original.status === 'Unpaid' ? 'bg-red-50 text-red-700' : 'bg-yellow-50 text-yellow-700'
         }`}>
             {row.original.status}
         </span>
      )
