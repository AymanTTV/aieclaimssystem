// src/components/vehicles/ManageVehicleDepartmentsModal.tsx
...
interface ManageVehicleDepartmentsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const ManageVehicleDepartmentsModal: React.FC<ManageVehicleDepartmentsModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchDepartments = async () => {
      const snap = await getDocs(collection(db, 'vehicleDepartments'));
  ...
  const handleAddDepartment = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const docRef = await addDoc(collection(db, 'vehicleDepartments'), { name: trimmed, createdAt: serverTimestamp() });
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    setLoading(true);
    try {
      await deleteDoc(doc(db, 'vehicleDepartments', id));
  ...
  const handleSaveEdit = async () => {
    ...
    setLoading(true);
    try {
      await updateDoc(doc(db, 'vehicleDepartments', editingId), { name: editingName.trim() });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Manage Vehicle Departments" size="md">
...
export default ManageVehicleDepartmentsModal;