...
const VehicleForm: React.FC<VehicleFormProps> = ({ vehicle, onClose, onSubmit }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    nextServiceMileage:
      vehicle?.nextServiceMileage?.toString() ?? ((vehicle?.mileage ?? 0) + 25000).toString(),
    ...
    weeklyInsuranceAmount:
      (vehicle as any)?.weeklyInsuranceAmount?.toString() ??
      DEFAULT_INSURANCE_AMOUNTS.weekly.toString(),
    dailyInsuranceAmount:
      (vehicle as any)?.dailyInsuranceAmount?.toString() ??
      DEFAULT_INSURANCE_AMOUNTS.daily.toString(),
    claimInsuranceAmount:
      (vehicle as any)?.claimInsuranceAmount?.toString() ??
      DEFAULT_INSURANCE_AMOUNTS.claim.toString(),
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-end space-x-2 pt-2">
        ...
        <button
          type="submit"
          disabled={loading}
          className="px-4 py-2 border border-transparent rounded-md shadow-sm text-white bg-primary hover:bg-primary-600 disabled:opacity-50"
        >
