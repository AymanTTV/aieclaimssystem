        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          {previews.map(({ url, size, type }, index) => (
            <div key={index} className="relative group">
              ...
              <div className="absolute inset-x-0 bottom-0 p-2 bg-black/60 text-white text-xs">
                {formatFileSize(size)}
              </div>
              <button
                type="button"
                onClick={() => removeFile(index)}
                className="absolute -top-2 -right-2 bg-red-600 border border-white rounded-full p-1 hover:bg-red-700 text-white shadow-sm"
              >
